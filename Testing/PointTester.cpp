...
void PointTester::testInvalid() {
    ...
    Point s5("mmm");
    if (s0.isValid()){
        std::cout << "Unexpectedly valid: (m, 0, 0)" << std::endl;
    }
    if (s1.isValid()){
        std::cout << "Unexpectedly valid: (0, m, 0)" << std::endl;
    }
    if (s2.isValid()){
        std::cout << "Unexpectedly valid: (0, 0, m)" << std::endl;
    }
    if (s3.isValid()){
        std::cout << "Unexpectedly valid: (m, m, m)" << std::endl;
    }
    if (s4.isValid()){
        std::cout << "Unexpectedly valid: ( , m, m, m)" << std::endl;
    }
    if (s5.isValid()){
        std::cout << "Unexpectedly valid: (mmm)" << std::endl;
    }
